...
// ---------------------------------------------------------------------------
// Security: slug and relative-path validation
// ---------------------------------------------------------------------------

/**
 * Slug regex: must start with an alphanumeric character and contain only
 * lowercase letters, digits, underscores, and hyphens.
 * Rejects: empty string, leading hyphen/underscore, any path separator,
 * any `..` traversal, and any characters outside [a-z0-9_-].
 */
const SLUG_RE = /^[a-z0-9][a-z0-9_-]*$/;

/**
 * Validate a slug (phase.id or spot.id).
 * Returns null on success, error message on failure.
 */
function validateSlug(value: string, label: string): string | null {
  if (!SLUG_RE.test(value)) {
    return `${label} '${value}' is invalid: must match /^[a-z0-9][a-z0-9_-]*$/`;
  }
  return null;
}

/**
 * Validate a relative-path field (target_file, hint_md, reference_md,
 * auto_write_md).  Rules:
 *   - Must not be an absolute path (no leading /).
 *   - Must not contain any `..` segment (path traversal).
 *   - Must not be empty.
 * Returns null on success, error message on failure.
 */
function validateRelativePath(value: string, label: string): string | null {
  ...
  if (value.startsWith('/') || value.startsWith('\\')) {
    return `${label} '${value}' must be a relative path, not absolute`;
  }
  // Split on both / and \ to catch mixed separators
  const segments = value.split(/[/\\]/);
  if (segments.includes('..')) {
    return `${label} '${value}' must not contain '..' segments`;
  }
  return null;
}
...
export function validatePhases(v: unknown): ValidationResult<PhasesData> {
  ...
  for (const phase of phases) {
    ...
    // Validate phase.id as a slug
    const phaseIdErr = validateSlug(phaseId, `Phase id`);
    if (phaseIdErr) {
      return { ok: false, error: phaseIdErr };
    }
    ...
    for (const spot of p['spots'] as unknown[]) {
      ...
      // Validate spot.id as a slug
      const spotIdErr = validateSlug(spotId, `Phase ${phaseId} spot id`);
      if (spotIdErr) {
        return { ok: false, error: spotIdErr };
      }
      ...
      if (hasAnyNewField) {
        ...
        // Validate target_file as a relative path (no absolute, no ..)
        const targetFileErr = validateRelativePath(s['target_file'] as string, `Phase ${phaseId} spot ${spotId} target_file`);
        if (targetFileErr) {
          return { ok: false, error: targetFileErr };
        }
        ...
        if (s['rungs'] !== undefined) {
          ...
          // Validate rung paths as relative paths (no absolute, no ..)
          const hintMdErr = validateRelativePath(r['hint_md'] as string, `Phase ${phaseId} spot ${spotId} rungs.hint_md`);
          if (hintMdErr) {
            return { ok: false, error: hintMdErr };
          }
          const refMdErr = validateRelativePath(r['reference_md'] as string, `Phase ${phaseId} spot ${spotId} rungs.reference_md`);
          if (refMdErr) {
            return { ok: false, error: refMdErr };
          }
          const autoMdErr = validateRelativePath(r['auto_write_md'] as string, `Phase ${phaseId} spot ${spotId} rungs.auto_write_md`);
          if (autoMdErr) {
            return { ok: false, error: autoMdErr };
          }
