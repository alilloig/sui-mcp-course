...
import { canAdvanceRung, recordRungUse, runAutoWrite, AutoWriteError, containedPath, PathTraversalError } from '../ladder.js';
...
export async function runRequestHint(
  ...
  // The base directory for rung content paths is <projectRoot>/paths/<slug>/
  const pathsSlugDir = path.join(projectRoot, 'paths', slug);

  let rungContentPath: string;
  if (rungRelPath) {
    // Containment-check: rungRelPath must not escape pathsSlugDir
    try {
      rungContentPath = containedPath(pathsSlugDir, rungRelPath);
    } catch (err) {
      return {
        ok: false,
        error: {
          kind: 'auto-write-failed',
          message: `auto-write-failed (path-traversal): ${(err as Error).message}`,
        },
      };
    }
  } else {
    // Fallback to canonical structure if spot.rungs not declared
    rungContentPath = path.join(pathsSlugDir, 'rungs', spot.id, rungFile);
  }
  ...
  try {
    ...
    if (err instanceof PathTraversalError) {
      return {
        ...
        error: {
          kind: 'auto-write-failed',
          message: `auto-write-failed (path-traversal): ${err.message}`,
        },
      };
    }
    const e = err as Error;
