...
// ---------------------------------------------------------------------------
// containedPath — security helper for rung content paths
// ---------------------------------------------------------------------------

/**
 * Resolve `untrusted` relative to `root` and assert the result is strictly
 * inside `root`. Returns the resolved absolute path on success, or null on
 * containment failure (dotdot traversal, absolute path injection, etc.).
 */
...
export async function runRequestHint(
  ...
  // Security: validate rungRelPath is contained within paths/<slug>/
  // The rung content base dir is projectRoot/paths/<slug>
  const pathSlugDir = path.join(projectRoot, 'paths', slug);

  let rungContentPath: string;
  if (rungRelPath) {
    // Validate the rungRelPath from the manifest is contained under the slug dir
    const safePath = containedPath(pathSlugDir, rungRelPath);
    if (safePath === null) {
      return {
        ok: false,
        error: {
          kind: 'auto-write-failed',
          message: `auto-write-failed (path-traversal): rung path '${rungRelPath}' resolves outside the path directory`,
        },
      };
    }
    rungContentPath = safePath;
  } else {
    // Fallback to canonical structure if spot.rungs not declared
    rungContentPath = path.join(pathSlugDir, 'rungs', spot.id, rungFile);
  }
