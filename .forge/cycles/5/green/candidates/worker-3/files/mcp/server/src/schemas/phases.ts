...
// ---------------------------------------------------------------------------
// Security: slug regex and path safety
// ---------------------------------------------------------------------------

/**
 * Valid slug: lowercase alphanumeric, may contain hyphens and underscores,
 * must start with alphanumeric. No dots, slashes, or other special characters.
 */
const SLUG_REGEX = /^[a-z0-9][a-z0-9_-]*$/;

/**
 * Check that a relative path does not contain dotdot segments or start with
 * a separator (absolute path injection).
 */
function isSafeRelativePath(p: string): boolean {
  if (!p) return false;
  // Reject absolute paths
  if (p.startsWith('/') || p.startsWith('\\') || /^[a-zA-Z]:/.test(p)) return false;
  // Reject dotdot segments
...
export function validatePhases(v: unknown): ValidationResult<PhasesData> {
  ...
  for (const phase of phases) {
    ...
    if (!SLUG_REGEX.test(phaseId)) {
      return {
        ok: false,
        error: `Phase id '${phaseId}' is invalid: must match /^[a-z0-9][a-z0-9_-]*$/`,
      };
    }
    ...
    for (const spot of p['spots'] as unknown[]) {
      ...
      if (!SLUG_REGEX.test(spotId)) {
        return {
          ok: false,
          error: `Phase ${phaseId} spot id '${spotId}' is invalid: must match /^[a-z0-9][a-z0-9_-]*$/`,
        };
      }
      ...
      if (hasAnyNewField) {
        ...
        // Security: reject target_file with dotdot or absolute path
        const targetFile = s['target_file'] as string;
        if (!isSafeRelativePath(targetFile)) {
          return {
            ok: false,
            error: `Phase ${phaseId} spot ${spotId}: target_file '${targetFile}' contains path traversal or is absolute`,
          };
        }
        ...
        spotData.target_file = targetFile;
        ...
        if (s['rungs'] !== undefined) {
          ...
          // Security: validate rung paths for dotdot/absolute
          const hintMd = r['hint_md'] as string;
          const referenceMd = r['reference_md'] as string;
          const autoWriteMd = r['auto_write_md'] as string;

          if (!isSafeRelativePath(hintMd)) {
            return {
              ok: false,
              error: `Phase ${phaseId} spot ${spotId}: rungs.hint_md '${hintMd}' contains path traversal or is absolute`,
            };
          }
          if (!isSafeRelativePath(referenceMd)) {
            return {
              ok: false,
              error: `Phase ${phaseId} spot ${spotId}: rungs.reference_md '${referenceMd}' contains path traversal or is absolute`,
            };
          }
          if (!isSafeRelativePath(autoWriteMd)) {
            return {
              ok: false,
              error: `Phase ${phaseId} spot ${spotId}: rungs.auto_write_md '${autoWriteMd}' contains path traversal or is absolute`,
            };
          }
