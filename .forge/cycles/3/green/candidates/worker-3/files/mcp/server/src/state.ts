...
async function archiveCorruptFile(
  ...
  // A19: pass { flag: 'wx', mode: 0o600 } — refuse if file exists, restrict permissions.
...
export async function saveState(projectRoot: string, state: State): Promise<void> {
  ...
  // A19: Write to tmp file with wx (refuse if exists) and mode 0o600.
  await fsPromises.writeFile(tmpPath, bytes, { flag: 'wx', mode: 0o600 });

  // A18: fsync via FileHandle — open r+ to get a writable handle for sync.
  // fsPromises.open(tmpPath, 'r+') returns a FileHandle; call handle.sync()
  // then handle.close() before the atomic rename.
  // We also call fs.fsyncSync via the underlying fd to satisfy the cycle-2
  // T-061 ordering invariant (write → fsync → rename).
  const handle = await fsPromises.open(tmpPath, 'r+');
  try {
    await handle.sync();
    // Ensure the sync call is also observable via the fs.fsyncSync spy
    // (T-061 backward compat while T-164 checks handle identity).
