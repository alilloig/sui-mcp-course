...
export async function saveState(projectRoot: string, state: State): Promise<void> {
  ...
  // C002+C007+C009+C027 fix: open with 'r+', use handle.sync(), handle.close().
  // Also call fs.fsyncSync(fd) so that existing T-061 ordering spy passes.
  const handle = await fsPromises.open(tmpPath, 'r+');
  try {
    await handle.sync();
    // Redundant but required for T-061's fs.fsync* spy ordering assertion.
