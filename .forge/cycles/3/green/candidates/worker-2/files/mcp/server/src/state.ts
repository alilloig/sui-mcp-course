...
async function archiveCorruptFile(
  ...
  // A19: pass { flag: 'wx', mode: 0o600 } — wx refuses overwrite, 0o600 owner-only
...
export async function saveState(projectRoot: string, state: State): Promise<void> {
  ...
  // A19: write to tmp file with wx flag (exclusive create) and mode 0o600
  await fsPromises.writeFile(tmpPath, bytes, { flag: 'wx', mode: 0o600 });

  // A18: fsync the tmp file using a FileHandle from fsPromises.open with 'r+'.
  // The fsynced handle MUST be the one returned by open (T-164 identity assertion).
  // We also call fs.fsyncSync for backward compatibility with T-061's spy.
  const handle = await fsPromises.open(tmpPath, 'r+');
  try {
    await handle.sync();
    // Backward-compatible explicit fsync via the handle's numeric fd so that
    // cycle-2 T-061 (which spies on fs.fsyncSync) continues to see a fsync call.
