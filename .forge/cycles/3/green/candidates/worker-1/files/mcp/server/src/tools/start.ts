...
import type { PathInfo } from '../registry.js';
import { loadState } from '../state.js';
import type { State } from '../state.js';
import type { EngineWarning, StateWarning } from '../warnings.js';
...
export async function runStart({ projectRoot }: { projectRoot: string }): Promise<StartResult> {
  ...
  if (!styleResult.ok) {
    const warnings: EngineWarning[] = [...registry.warnings];
    if (styleResult.warning) {
      // Cast outputStyle warning to EngineWarning (it's structurally compatible)
      warnings.unshift(styleResult.warning as unknown as EngineWarning);
    }
    return {
      outputStyleOk: false,
      preflight: { skipped: true, reason: 'cycle-1' },
