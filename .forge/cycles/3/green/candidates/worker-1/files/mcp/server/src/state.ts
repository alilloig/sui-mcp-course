...
async function archiveCorruptFile(
  ...
  // A19: use { flag: 'wx', mode: 0o600 } to refuse overwrite and restrict permissions
...
export async function saveState(projectRoot: string, state: State): Promise<void> {
  ...
  // A19: write to tmp file with wx flag (refuse overwrite) and restricted permissions
  await fsPromises.writeFile(tmpPath, bytes, { flag: 'wx', mode: 0o600 });

  // A18: fsync via FileHandle returned by fsPromises.open with 'r+' flag
  // The FileHandle identity is asserted by T-164: the synced handle must be
  // the exact same object returned by open.
