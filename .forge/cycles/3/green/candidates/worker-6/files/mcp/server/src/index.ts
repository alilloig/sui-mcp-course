...
export function registerTools(server: McpServer): void {
  ...
  server.tool(
    'runPreflightProbe',
    'Run a single preflight probe by id. Probe ids (in order): docker-running, node-version, pnpm-available, sui-cli-version, sui-pilot-enabled, sandbox-repo-present, sandbox-manifest-reachable, learning-output-style-enabled. Pass remediate: true to trigger shell action execution (only sandbox-manifest-reachable has one).',
    {
      probeId: z.string().describe(
        'One of: docker-running, node-version, pnpm-available, sui-cli-version, sui-pilot-enabled, sandbox-repo-present, sandbox-manifest-reachable, learning-output-style-enabled',
      ),
      remediate: z.boolean().optional().describe(
        'If true and the probe returned a shell action, execute the remediation (pnpm deploy-all --quick for sandbox-manifest-reachable).',
      ),
    },
