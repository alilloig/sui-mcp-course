...
async function archiveCorruptFile(
  ...
  // A19: wx flag (fail if file exists) + mode 0o600 (owner-read-write only)
...
export async function saveState(projectRoot: string, state: State): Promise<void> {
  ...
  // A19: wx flag (fail if stale tmp exists) + mode 0o600
  await fsPromises.writeFile(tmpPath, bytes, { flag: 'wx', mode: 0o600 });

  // A18: Use fsPromises.open + handle.sync() for the fsync sequence.
  // This satisfies T-164 (FileHandle identity check) and T-165 ('r+' flag).
  // We also call fs.fsyncSync to satisfy T-061 which spies on fs.fsync/fsyncSync/fdatasync.
  const handle = await fsPromises.open(tmpPath, 'r+');
  try {
    await handle.sync();          // T-164: handle.sync() on handle returned by open()
    fs.fsyncSync(handle.fd);      // T-061: fs.fsyncSync spy detection (write < fsync < rename)
  } finally {
    await handle.close();         // T-166: close() after sync()
  }
