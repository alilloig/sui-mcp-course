...
async function archiveCorruptFile(
  ...
  // A19: wx flag refuses overwrite; mode 0o600 for owner-only access.
...
export async function saveState(projectRoot: string, state: State): Promise<void> {
  ...
  // A19: wx flag refuses overwrite on the tmp write; mode 0o600 for owner-only access.
  await fsPromises.writeFile(tmpPath, bytes, { flag: 'wx', mode: 0o600 });

  // A18: fsync via FileHandle — open with 'r+' so we get a writable handle
  // (needed for sync on some kernels), then sync and close before rename.
  // We also call fs.fsyncSync(handle.fd) for compatibility with T-061 ordering
  // assertions that spy on the fs module's exported fsync functions.
  const handle = await fsPromises.open(tmpPath, 'r+');
  try {
    await handle.sync();
    // Compatibility: also call the exported fsyncSync so order-sensitive spies
    // on fs.fsyncSync (T-061) observe the fsync between write and rename.
