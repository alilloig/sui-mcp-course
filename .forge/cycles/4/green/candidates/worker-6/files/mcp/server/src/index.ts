...
export function registerTools(server: McpServer): void {
  ...
  server.tool(
    'selectPath',
    'Select a learning path by slug. Returns personalization prompts for the selected path and initializes the cursor to the first phase/spot.',
    {
      projectRoot: z.string().describe('Absolute path to the project root'),
      slug: z.string().describe('The path slug to select (e.g. "01-orderbook-viewer")'),
    },
  ...
  server.tool(
    'setPersonalization',
    'Set personalization values for the selected learning path. Pass an empty object to use all defaults.',
    {
      projectRoot: z.string().describe('Absolute path to the project root'),
      values: z.record(z.unknown()).describe('Personalization key-value pairs'),
    },
    async ({ projectRoot, values }) => {
      const result = await setPersonalization({ projectRoot, values });
      return {
        content: [
          {
            type: 'text' as const,
  ...
  server.tool(
    'nextSpot',
    'Get the current phase and spot for the active learning path. Returns the spot with substituted prompt and doc links.',
    {
      projectRoot: z.string().describe('Absolute path to the project root'),
    },
    async ({ projectRoot }) => {
  ...
  server.tool(
    'verifySpot',
    'Verify the current spot in the active learning path. On pass, advances the cursor to the next spot.',
    {
      projectRoot: z.string().describe('Absolute path to the project root'),
    },
    async ({ projectRoot }) => {
