...
export function validateState(v: unknown): ValidationResult<State> {
  ...
  // Normalize ladder rungs: add auto_write_attempted: false if absent.
  const rawLadder = obj['ladder'] as Record<string, unknown>;
  const ladder: Record<string, LadderRung> = {};
  for (const [spotId, rungRaw] of Object.entries(rawLadder)) {
    if (typeof rungRaw !== 'object' || rungRaw === null) {
      ladder[spotId] = {
        hint_used: false,
      ...
      const r = rungRaw as Record<string, unknown>;
      ladder[spotId] = {
