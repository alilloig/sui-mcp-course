...
import { validatePhasesStructure } from './schemas/phases.js';
...
export async function scanRegistry(scanRoot: string): Promise<RegistryResult> {
  ...
  for (const entry of dirEntries) {
    ...
    // Note: registry uses validatePhasesStructure (structural/lenient) rather
    // than validatePhases (strict). The strict validation happens at phase
    // engine load time; the registry only ensures the file parses and has
    // non-empty spots — this preserves backward compat with cycle-1 fixtures.
    ...
    const phasesValidation = validatePhasesStructure(phasesParsed);
