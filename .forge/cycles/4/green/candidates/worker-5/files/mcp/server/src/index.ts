...
export function registerTools(server: McpServer): void {
  ...
  server.tool(
    'selectPath',
    'Select a learning path by slug. Initializes state.cursor and returns personalizationPrompts.',
    {
      projectRoot: z.string().describe('Absolute path to the project root'),
  ...
  server.tool(
    'setPersonalization',
    'Set personalization values for the selected path. Pass an empty object to use defaults.',
    {
      projectRoot: z.string().describe('Absolute path to the project root'),
      values: z.record(z.unknown()).describe('Personalization values keyed by option name'),
    },
  ...
  server.tool(
    'nextSpot',
    'Get the current phase/spot/ladder for the active cursor. Returns done:true when the path is exhausted.',
    {
      projectRoot: z.string().describe('Absolute path to the project root'),
    },
    async ({ projectRoot }) => {
