...
export async function scanRegistry(scanRoot: string): Promise<RegistryResult> {
  ...
  for (const entry of dirEntries) {
    ...
    // Use structural validation for registry scanning — only checks non-empty
    // phases/spots arrays and string ids. Full field validation (target_file,
    // target_range, prompt, verification) is enforced by loadPhases at runtime.
