...
export function validateState(v: unknown): ValidationResult<State> {
  ...
  // Normalize ladder rungs to include auto_write_attempted (default false)
  ...
  for (const [key, rung] of Object.entries(rawLadder)) {
    if (typeof rung === 'object' && rung !== null && !Array.isArray(rung)) {
      ...
      ladder[key] = { hint_used: false, reference_shown: false, auto_completed: false, auto_write_attempted: false };
    }
  }
