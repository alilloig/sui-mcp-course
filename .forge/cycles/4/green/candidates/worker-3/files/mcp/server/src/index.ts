...
export function registerTools(server: McpServer): void {
  ...
  server.tool(
    'selectPath',
    'Select a learning path by slug. Returns personalization prompts. Initializes the cursor at the first phase/spot.',
    {
      projectRoot: z.string().describe('Absolute path to the project root'),
      slug: z.string().describe('Path slug (e.g. the directory name under paths/)'),
    },
  ...
  server.tool(
    'setPersonalization',
    'Set personalization values for the selected path. Pass an empty object to accept all defaults.',
    {
      projectRoot: z.string().describe('Absolute path to the project root'),
      values: z.record(z.unknown()).describe('Personalization key-value pairs'),
    },
    async ({ projectRoot, values }) => {
      const result = await runSetPersonalization({ projectRoot, values });
      return {
        content: [
          {
            type: 'text' as const,
  ...
  server.tool(
    'nextSpot',
    'Get the current phase and spot with substituted prompt. Returns done:true when the path is complete.',
    {
      projectRoot: z.string().describe('Absolute path to the project root'),
    },
    async ({ projectRoot }) => {
  ...
  server.tool(
    'verifySpot',
    'Run verification for the current spot. On pass, advances the cursor. Returns { pass, output, advanced }.',
    {
      projectRoot: z.string().describe('Absolute path to the project root'),
    },
    async ({ projectRoot }) => {
