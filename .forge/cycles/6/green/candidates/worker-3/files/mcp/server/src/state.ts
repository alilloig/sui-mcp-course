...
// Compute a short hex hash of the content for dedup checking.
...
async function archiveCorruptFile(
  ...
  // H003 dedup: check if an archive for this exact content already exists.
  // We embed the content hash in the archive filename and scan for it before
  // writing. If found, return the path to the existing archive immediately
  ...
  const existing = entries.find(
    (name) => name.startsWith('state.corrupt-') && name.includes(hash) && name.endsWith('.json'),
  );
  if (existing) {
    return path.join(stateDir, existing);
  }
  ...
  archiveCounter += 1;
  const suffix = `${ts}-${archiveCounter}-${hash}`;
