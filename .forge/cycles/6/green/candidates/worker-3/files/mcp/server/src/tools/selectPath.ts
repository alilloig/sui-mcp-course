...
export async function runSelectPath({
  ...
  if (stateResult.kind === 'corrupt') {
    // H003 / AC-7.2: if the corrupt state was successfully archived (archivedTo
    // is defined), treat the slot as absent and mint a fresh state below.
    // If archivedTo is undefined (archive write failed or read-error path),
    // we cannot safely proceed — return the error as before.
    if (stateResult.archivedTo === undefined) {
      return { ok: false, errors: [`State corrupt: ${stateResult.message}`] };
    }
    // Fall through to the fresh-state mint path below (stateResult.kind === 'corrupt'
    // with archivedTo is treated the same as 'absent').
  }
  ...
  // Build new state
  // For corrupt+archivedTo, treat as absent (no existing state to carry forward).
  const existingState =
