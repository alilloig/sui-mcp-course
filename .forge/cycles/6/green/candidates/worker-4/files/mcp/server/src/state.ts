...
/**
 * Compute a short SHA-256 hex digest of a string. Used for deduplication:
 * if an archive file with the same content hash already exists in stateDir,
 * we return that archive path rather than creating a second identical file.
 */
...
/**
 * Find an existing archive file in stateDir whose content matches the given
 * raw string (compared via hash). Returns the full path if found, or null.
 */
async function findExistingArchive(stateDir: string, raw: string): Promise<string | null> {
  const hash = contentHash(raw);
  let entries: string[];
  try {
    entries = await fsPromises.readdir(stateDir);
  } catch {
    return null;
  }

  for (const name of entries) {
    if (!/^state\.corrupt-.*\.json$/.test(name)) continue;
    const archivePath = path.join(stateDir, name);
    try {
      const existing = await fsPromises.readFile(archivePath, 'utf8');
      if (contentHash(existing) === hash && existing === raw) {
        return archivePath;
      }
    } catch {
      // Can't read this archive — skip it
    }
  }
  return null;
}
...
async function archiveCorruptFile(
  ...
  // H003 dedup: if an archive with identical bytes already exists, return its
  // path instead of creating a second copy. This prevents N archives from
  // accumulating across repeated /start calls on the same corrupt input.
  const existingPath = await findExistingArchive(stateDir, content);
