...
export async function runSelectPath({
  ...
  // Load state — handle corrupt/schema-mismatch
  const stateResult = await loadState(projectRoot);

  // H003 fix: when kind === 'corrupt' AND archivedTo is defined, the corrupt
  // bytes have been safely archived; treat the slot as 'absent' so we can
  // mint a fresh State via saveState below.
  if (stateResult.kind === 'corrupt') {
    if (stateResult.archivedTo === undefined) {
      // Archive failed — cannot safely recover; surface the error.
      return { ok: false, errors: [`State corrupt: ${stateResult.message}`] };
    }
    // archivedTo is defined — fall through to mint fresh state (treat as absent).
  ...
  // Build new state — when stateResult.kind === 'corrupt' + archivedTo, treat
  // as absent (existingState is null).
  const existingState =
