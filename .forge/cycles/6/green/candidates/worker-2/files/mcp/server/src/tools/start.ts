...
export interface StartResult {
  ...
  paths: PathInfo[];
  state: State | null;
  warnings: (RegistryWarning | StateWarning | OutputStylePluginNotEnabledWarning | { kind: string; message: string })[];
}
...
export async function runStart({ projectRoot }: { projectRoot: string }): Promise<StartResult> {
  ...
  if (!styleResult.ok) {
    const warnings: StartResult['warnings'] = [...registry.warnings];
