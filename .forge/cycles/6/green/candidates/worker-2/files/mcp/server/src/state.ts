...
async function archiveCorruptFile(
  ...
  // H003: De-duplicate archives by content hash. If an archive with identical
  // bytes already exists in stateDir, return its path without writing a new one.
  const contentHash = crypto.createHash('sha256').update(content, 'utf8').digest('hex');
  let existingFiles: string[];
  try {
    existingFiles = await fsPromises.readdir(stateDir);
  } catch {
    existingFiles = [];
  }
  ...
  for (const name of existingFiles) {
    ...
    const candidatePath = path.join(stateDir, name);
    try {
      const candidateContent = await fsPromises.readFile(candidatePath, 'utf8');
      const candidateHash = crypto.createHash('sha256').update(candidateContent, 'utf8').digest('hex');
      if (candidateHash === contentHash) {
        // Identical content already archived — return existing path.
        return candidatePath;
      }
    } catch {
      // If we can't read a candidate, skip it.
    }
  }
