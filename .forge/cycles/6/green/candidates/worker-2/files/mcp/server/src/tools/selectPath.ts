...
export async function runSelectPath({
  ...
  // Load state — handle corrupt+archivedTo by treating slot as absent (H003 fix)
  const stateResult = await loadState(projectRoot);
  if (stateResult.kind === 'corrupt') {
    // H003/AC-7.2: if the corrupt file has been archived, treat the slot as
    // absent and proceed to mint a fresh state. If archivedTo is undefined
    // (archive failed), the bytes are unreadable and we must still fail safely.
    if (stateResult.archivedTo === undefined) {
      return { ok: false, errors: [`State corrupt: ${stateResult.message}`] };
    }
    // archivedTo is defined: fall through as if absent (treat as 'absent')
  ...
  // Build new state — use existing state data when available (ok), otherwise start fresh
